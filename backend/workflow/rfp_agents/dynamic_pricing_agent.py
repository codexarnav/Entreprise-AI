...
class AgentState(TypedDict):
    ...
    # Additional Context (for estimation when no SKU matches)
    technical_gaps: Optional[List[Dict]]  # Technical gaps from Technical Agent
    rfp_requirements: Optional[List[Dict]]  # RFP requirements for estimation
...
def estimate_pricing_from_gaps(state: AgentState) -> float:
    """
    Estimate pricing when no SKU matches are available.
    Uses technical gaps, RFP scope, and other factors to estimate cost.
    """
    # Try to extract information from state
    ...
    base_estimate = 50000.0  # Base project cost
    
    # Adjust based on number of gaps (more gaps = more innovation needed = higher cost)
    if technical_gaps:
        gap_multiplier = 1.0 + (len(technical_gaps) * 0.1)
        base_estimate *= gap_multiplier
    
    # Adjust based on number of requirements
    if rfp_requirements:
        req_multiplier = 1.0 + (len(rfp_requirements) * 0.05)
        base_estimate *= req_multiplier
    
    # Adjust based on PWin (lower PWin might indicate more complex/risky project)
    if pwin_score < 50:
        base_estimate *= 1.2  # 20% more for risky projects
...
def calculate_costs_with_risk_buffers(state: AgentState) -> AgentState:
    ...
    - Low Match (<70%): Standard_Cost + 25% Risk Buffer
    - Innovation Item: Standard_Cost + NRE_Cost
    - Unmatched: Estimate pricing instead of failing
    """
    sku_matches = state.get("sku_matches", [])
    price_book = state.get("price_book", {})
    ...
    print(f"  📊 Pricing Input: {len(sku_matches)} SKU matches, {len(price_book)} price book entries")
    
    # Handle empty SKU matches - create estimated pricing based on other factors
    if not sku_matches:
        # Estimate pricing based on technical gaps, RFP scope, and other factors
        # This allows pricing even when no SKUs are matched
        estimated_base_cost = estimate_pricing_from_gaps(state)
        
        if estimated_base_cost > 0:
            risk_notes.append(
                "No SKU matches found. Pricing estimated based on technical requirements and gaps."
            )
            
            # Create a single estimated line item
            estimated_item = {
                "item_number": 1,
                "sku_id": "ESTIMATED-001",
                "sku_name": "Estimated Solution Package",
                "match_confidence_score": 0.0,  # No match
                "base_cost": estimated_base_cost,
                "risk_buffer": estimated_base_cost * 0.25,  # Higher buffer for estimates
                "innovation_cost": estimated_base_cost * 0.15,  # Innovation cost for gaps
                "adjusted_cost": estimated_base_cost * 1.40,  # Base + buffer + innovation
                "quantity": 1,
            }
            
            priced_items.append(estimated_item)
            total_base_cost = estimated_base_cost
            total_risk_buffer = estimated_base_cost * 0.25
            total_innovation_cost = estimated_base_cost * 0.15
            ...
            # If estimation also fails, use a default minimum
            risk_notes.append(
                "No SKU matches and estimation unavailable. Using minimum project pricing."
            )
            default_cost = 10000.0  # Minimum project cost
            
            default_item = {
                "item_number": 1,
                "sku_id": "DEFAULT-001",
                "sku_name": "Default Project Package",
                "match_confidence_score": 0.0,
                "base_cost": default_cost,
                "risk_buffer": default_cost * 0.30,
                "innovation_cost": default_cost * 0.20,
                "adjusted_cost": default_cost * 1.50,
                "quantity": 1,
            }
            
            priced_items.append(default_item)
            state["priced_items"] = priced_items
            state["total_base_cost"] = default_cost
            state["total_risk_buffer"] = default_cost * 0.30
            state["total_innovation_cost"] = default_cost * 0.20
            state["critical_errors"] = critical_errors
            state["risk_notes"] = risk_notes
            return state
    
    # Handle empty price book - but only if we have SKU matches that need pricing
    # If we have SKU matches but no price book, estimate pricing for them
    if not price_book and sku_matches:
        risk_notes.append(
            "Price book is empty. Estimating costs for matched SKUs based on standard rates."
        )
        # Create estimated price book from SKU matches
        for match in sku_matches:
            sku_id = match.get("sku_id", "")
            if sku_id and sku_id not in price_book:
                # Estimate cost based on match confidence (higher confidence = higher base cost estimate)
                confidence = match.get("match_confidence_score", 50.0)
                estimated_cost = 1000.0 + (confidence * 10)  # Base $1000 + $10 per confidence point
                price_book[sku_id] = estimated_cost
        print(f"  ℹ️  Created estimated price book with {len(price_book)} entries")
    ...
    for match in sku_matches:
        ...
        # Check if unmatched (no price book entry) - estimate instead of failing
        if standard_cost is None:
            # Estimate cost instead of failing
            confidence = match_confidence if match_confidence > 0 else 50.0
            estimated_cost = 1000.0 + (confidence * 10)  # Base $1000 + $10 per confidence point
            standard_cost = estimated_cost
            price_book[sku_id] = estimated_cost  # Add to price book for future reference
            risk_notes.append(
                f"SKU '{sku_name}' (SKU: {sku_id}) not in price book. Estimated cost: ${estimated_cost:,.2f}"
            )
        ...
        if match_confidence > 95:
            ...
            # Low confidence (<70%): Apply higher risk buffer instead of failing
            risk_buffer = base_cost * 0.25  # 25% buffer for low confidence
            risk_notes.append(
                f"Warning: SKU '{sku_name}' (SKU: {sku_id}) has low match confidence "
                f"({match_confidence:.1f}%). Applied 25% risk buffer."
            )
...
def calculate_final_pricing(state: AgentState) -> AgentState:
    ...
    """
    # Check for critical errors - but only fail if they're truly critical
    critical_errors = state.get("critical_errors", [])
    if critical_errors:
        # Filter out non-critical errors (warnings vs actual blockers)
        blocking_errors = [e for e in critical_errors if "CRITICAL_PRICING_ERROR" in e and "not found in price book" not in e]
        if blocking_errors:
            print(f"  ⚠️  Blocking errors found: {blocking_errors}")
    ...
    if not priced_items:
        print(f"  ⚠️  No priced items found. State keys: {list(state.keys())}")
        print(f"  ⚠️  Critical errors: {state.get('critical_errors', [])}")
        print(f"  ⚠️  Risk notes: {state.get('risk_notes', [])}")
        # Try to create a fallback pricing if we have any data
        if state.get("total_base_cost", 0) > 0:
            # We have costs but no items - create a single summary item
            fallback_item = {
                "item_number": 1,
                "sku_id": "SUMMARY-001",
                "sku_name": "Project Summary",
                "match_confidence_score": 0.0,
                "base_cost": state.get("total_base_cost", 0),
                "risk_buffer": state.get("total_risk_buffer", 0),
                "innovation_cost": state.get("total_innovation_cost", 0),
                "adjusted_cost": state.get("total_base_cost", 0) + state.get("total_risk_buffer", 0) + state.get("total_innovation_cost", 0),
                "quantity": 1,
            }
            priced_items = [fallback_item]
            state["priced_items"] = priced_items
        else:
            state["total_price"] = None
            state["pricing_table"] = []
