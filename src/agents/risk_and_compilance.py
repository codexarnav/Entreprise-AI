...
def generate_report(state: RiskAndComplianceState) -> RiskAndComplianceState:
    """Summarize extracted legal entities and assess risk."""
    ...
    TASK:
    - Summarize potential legal/compliance risks.
    - Identify risky clauses (e.g., indemnity, liability, warranty, penalties).
    - Assign a severity score between 0 and 1.
...
def access_risk(state: RiskAndComplianceState) -> RiskAndComplianceState:
    """Extract severity score from the JSON report."""
    import json

    try:
        parsed = json.loads(state["report"])
        score = float(parsed.get("severity", 0.0))
    except:
        score = 0.5  
